...
String sql_empty="";
table.append("<table class='altrowstable' id='alternatecolor'><tr><th>id</th><th>题目</th><th>查看</th><th>删除</th></tr>");
if(type.equals("sub")){
		...
		if(showPage<pageCount){
			for(int i=0;i<pageSize;i++){
				...
				table.append("<tr><td><input type='hidden' value='"+id+"' class='sub_id'>"+id+"</td><td>"+des+
						"</td><td><a href='show_ques.jsp?ptype=sub&pid="+id+"'target='_blank'>查看</a></td><td><input type='button' name='delete' value='删除' onclick='delete_sub(this)'></td></tr>");
				rs1.next();
			}
		}
		...
		if(showPage<pageCount){
			for(int i=0;i<pageSize;i++){
				...
				table.append("<tr><td><input type='hidden' value='"+id+"' class='cho_id'>"+id+"</td><td>"+des+
						"</td><td><a href='show_ques.jsp?ptype=cho&pid="+id+"'target='_blank'>查看</a></td><td><input type='button' name='delete' value='删除' onclick='delete_cho(this)'></td></tr>");
				rs1.next();
			}
		}
		else if(showPage==pageCount){
		...
		if(showPage<pageCount){
			for(int i=0;i<pageSize;i++){
				...
				table.append("<tr><td><input type='hidden' value='"+id+"' class='fill_id'>"+id+"</td><td>"+des+
						"</td><td><a href='show_ques.jsp?ptype=fill&pid="+id+"'target='_blank'>查看</a></td><td><input type='button' name='delete' value='删除' onclick='delete_fill(this)'></td></tr>");
				rs1.next();
			}
		}
		else if(showPage==pageCount){
...
<br>
	...
	<br>
	<a href="ques_list_manage.jsp?showPage=1&type=<%=type%>&search_ques=<%=key%>&point1=<%=point1%>&point2=<%=point2%>&diff=<%=diff%>">首页</a>
	<a href="ques_list_manage.jsp?showPage=<%=showPage-1%>&type=<%=type%>&search_ques=<%=key%>&point1=<%=point1%>&point2=<%=point2%>&diff=<%=diff%>">上一页</a>
<%	//根据pageCount的值显示每一页的数字并附加上相应的超链接
		for(int i=1;i<=pageCount;i++){
	%>
			<a href="ques_list_manage.jsp?showPage=<%=i%>&type=<%=type%>&search_ques=<%=key%>&point1=<%=point1%>&point2=<%=point2%>&diff=<%=diff%>"><%=i%></a>
<%	}
	%>	
	<a href="ques_list_manage.jsp?showPage=<%=showPage+1%>&type=<%=type%>&search_ques=<%=key%>&point1=<%=point1%>&point2=<%=point2%>&diff=<%=diff%>">下一页</a>
	<a href="ques_list_manage.jsp?showPage=<%=pageCount%>&type=<%=type%>&search_ques=<%=key%>&point1=<%=point1%>&point2=<%=point2%>&diff=<%=diff%>">末页</a>
</div>	
</body>
</html>