...
%>
<%!String add_obj_ques_fill="";

String add_obj_ans_fill="";
int obj_id=0;%>
...
if(request.getParameter("preview1")!=null) {//预览问题描述
	try{
		add_obj_ques_fill=request.getParameter("describe");
		
		add_obj_ans_fill=request.getParameter("answer");
...
if(request.getParameter("save1")!=null) {//保存描述
	...
	if(x.equals("")||y.equals("")){
		...
		try{
			...
			a.add_ques_fill(request.getParameter("describe").replace("\\","\\\\").replaceAll("<p>", "").replaceAll("</p>", ""),
			...
			/* String fmt="insert into objective_question_info_fill(obj_question_describe,obj_question_answer,obj_question_point) values('%s','%s','%s')";
			String sql=String.format(fmt,add_obj_ques_fill,request.getParameter("answer").replace("\\","\\\\"),request.getParameter("knowledge_point").replace("\\","\\\\"));
...
<script>
window.UMEDITOR_HOME_URL = "${base}/umeditor/";/** "相对于网站根目录的相对路径"**/
var um = UM.getEditor('container', {
    imageUrl:'${base}/uploadImages',/**图片上传执行action**/
    imagePath:'/upload/',/**图片网络显示路径**/
    focus: true
});
</script>
<script type="text/javascript">
...
<br>
<div>
<form action="add_question_obj_fill.jsp" method="post">
输入题目描述：
